		    ...
		    afs_status_p st)
{
    ...
    afs_status_t tst = 0;
    int code;
    struct sockaddr_in saddr = xxx_rx_CreateSockAddr(handle->ipAddr, handle->udpPort);
		       ...
		       int *last_item_contains_data, afs_status_p st)
{
    ...
    struct sockaddr_in saddr = xxx_rx_CreateSockAddr(t->handle->ipAddr, t->handle->udpPort);

    /*
     * Get the next entry the list of connections
     */
    code =
	rx_GetServerConnections(t->handle->sock, (struct sockaddr *)&saddr, 
                                &t->index, t->allconns,
		       ...
		       int *last_item_contains_data, afs_status_p st)
{
     ...
     */
    code =
	rx_GetServerPeers(t->handle->sock, (struct sockaddr *)&saddr, &t->index,
