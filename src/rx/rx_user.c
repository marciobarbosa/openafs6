...
# endif
    ...
    if (ntohs(saddr->addr.sh.port) >= IPPORT_RESERVED && ntohs(saddr->addr.sh.port) < IPPORT_USERRESERVED) {
/*	(osi_Msg "%s*WARNING* port number %d is not a reserved port number.  Use port numbers above %d\n", name, port, IPPORT_USERRESERVED);
*/ ;
    }
    if (ntohs(saddr->addr.sh.port) > 0 && ntohs(saddr->addr.sh.port) < IPPORT_RESERVED && geteuid() != 0) {
	(osi_Msg
	 "%sport number %d is a reserved port number which may only be used by root.  Use port numbers above %d\n",
	 name, ntohs(saddr->addr.sh.port), IPPORT_USERRESERVED);
	goto error;
    }
#endif
    socketFd = socket(saddr->addr.family, SOCK_DGRAM, IPPROTO_UDP);
...
static afs_uint32
fudge_netmask(struct rx_sockaddr *saddr) /* ipv4 only */
{
    afs_uint32 msk;
    rx_in_addr_t ipv4;

    if (!rx_try_sockaddr_to_ipv4(saddr, &ipv4))
        return EAFNOSUPPOT;
...
rxi_InitPeerParams(struct rx_peer *pp) /* ipv4 only */
{
    ...
    for (ix = 0; ix < rxi_numNetAddrs; ++ix) {
        rx_try_sockaddr_to_ipv4(&rxi_NetAddrs[ix], &ipv4);
        rx_try_sockaddr_to_ipv4(&myNetMasks[ix], &mask);
